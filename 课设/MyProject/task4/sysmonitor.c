...
void read_stat(char (*info)[1024], char *stat_file) {
   ...
   *
   * stat_file指针指向第一位, 依次读取pid, name, status, ppid, priority, memory use, 然后放入info中.
   */
  int pos;
 ...
 * 使用上面的read_stat函数获取信息, 并将信息显示到窗口中
 */
void get_process_info(void) {
  ...
  dir = opendir("/proc");
  process_num = 0;
  while (dir_info = readdir(dir)) {//依次读取dir下的每一个目录
    /*
     * 如果以数字开头, 则读取相应信息
     */
    if ((dir_info->d_name)[0] >= '0' && ((dir_info->d_name)[0]) <= '9') {
      ...
      close(fd);
      one_file = stat_file;
      read_stat(info, one_file);//获取进程相关信息
      for (int i = 0; i < 6; i++)
        txt[i] = utf8_fix(info[i]);
      gtk_clist_append(GTK_CLIST(clist), txt);//将获取到的信息显示到clist中
      process_num++;
    }
  }
  closedir(dir);
...
void get_modules_info(void) {//获取模块信息
  FILE *fp;
  ...
  while ((line = fgets(modules_info, sizeof(modules_info), fp)) != NULL) {
    ...
    gtk_clist_append(GTK_CLIST(clist2), txt);//将上述信息加入到clist2中
  }
  fclose(fp);
}
...
void get_cpu_info(char *cpu_name,
  ...
  /* Read CPU Name */
  i = 0;
  pos = strstr(info_buf, "model name");//找到model name
  while (*pos != ':')
    pos++;
  pos += 2;

  while (*pos != '\n') {//复制model name到info_str中
    info_str[i] = *pos;
    i++;
    pos++;
  }
  ...
  /* Read Cache size */
  i = 0;
  pos = strstr(info_buf, "cache size");//找到cache size
  while (*pos != ':')
    pos++;
  pos += 2;
  while (*pos != '\n') {
  ...
  /* Read CPU Cores */
  i = 0;
  pos = strstr(info_buf, "cpu cores");//找到cpu cores
  while (*pos != ':')
    pos++;
  pos += 2;
  while (*pos != '\n') {
  ...
  i = 0;
  pos = strstr(info_buf, "address sizes");//找到address sizes
  while (*pos != ':')
    pos++;
  pos += 2;
  while (*pos != '\n') {
...
void search_proc(GtkButton *button, gpointer data) {
  ...
  gchar *txt;
  gint ret, i = 0;
  entry_txt = gtk_entry_get_text(GTK_ENTRY(entry));//获取带搜索的进程号
  while ((ret = gtk_clist_get_text(GTK_CLIST(clist), i, 0, &txt)) != 0) {//获取clist中第i行第0列的数据
 ...
 */
void refresh_proc(void) {//冻结, 清空, 重新获取, 解冻显示, 跳转到第一个
